...
# --checkpoints_dir=./results/Q-Diffusion/ft_openvla/seed_0/ddpm_checkpoints_from_agent_trainer
# --instruction="put cup on pink plate" --repeat=4
...
def openvla_caching_worker(
    ...
    assert checkpoints_dir.endswith(
        "ddpm_checkpoints_from_agent_trainer"
    ) or checkpoints_dir.startswith("base_openvla:")
    ...
    while True:
        if (
            checkpoints_dir.startswith("base_openvla:")
            and current_checkpoint_index == -1
        ):
            all_episodes = glob(
                os.path.join(checkpoints_dir.split(":")[1], "*.tfrecord")
            )
            # Get the onces that are worker_id modulo num_workers
            episodes_left_to_cache = [
                episode
                for episode in all_episodes
                if int(episode.split("_")[-1].split(".")[0]) % num_workers == worker_id
            ]
        ...
        if current_checkpoint_index != -1 and len(episodes_left_to_cache) > 0:
            ...
            episode_rewards = episode["rewards"].numpy()
            ...
            #         all_observation_images, repeat=repeat, timer=timer
            #     )
            # else:
            ...
            #             timer=timer,
            #         )
            for i in range(len(all_observation_images)):
                if len(instructions) > 1 and i < len(episode_rewards):
                    assert episode_rewards[i] <= 0
                    print(
                        f"Setting instruction to: {instructions[int(episode_rewards[i])]}"
                    )
                    openvla_agent.set_language_instruction(
                        instructions[int(episode_rewards[i])]
                    )
                openvla_agent.sample_actions(
                    all_observation_images[i], repeat=repeat, timer=timer
                )

            # Save cache
            if checkpoints_dir.startswith("base_openvla:"):
                checkpoints_dir = checkpoints_dir.split(":")[1]
            ...
            # Make with 777 permissions
            os.makedirs(os.path.dirname(cache_save_path), exist_ok=True, mode=0o777)
            ...
            if (
                checkpoints_dir.startswith("base_openvla:")
                and len(episodes_left_to_cache) == 0
            ):
                print(f"Worker {worker_id} has finished caching all episodes")
                break
