...
class OpenVLAAgent:
    ...
    def openvla_normalize_actions(
        ...
        # action_norm_stats = self.vla_model.module.get_action_stats(norm_key)
        ...
        if cache:
            for i, observation in enumerate(observations):
                ...
                if observation_hash in self.action_cache:
                    # We might compute (repeat * multiplier) actions for each observation to avoid overfitting
                    ...
                    images: List[Image.Image] = (
                        [Image.fromarray(observation)] * repeat * multiplier
                    )
                    inputs = self.processor(
                        [self.prompt] * repeat * multiplier, images
            ...
            assert len(repeated_images) == batch_size * repeat * multiplier

            inputs = self.processor(
                [self.prompt] * batch_size * repeat * multiplier, repeated_images
            ).to(self.device_id, dtype=torch.bfloat16)
            if timer is not None:
                timer.tock("openvla_process_images")
            ...
            if hasattr(self.vla_model, "predict_action"):
                actions = self.vla_model.predict_action(
                    **inputs, unnorm_key="bridge_orig", do_sample=(not argmax)
                ).reshape(batch_size, repeat * multiplier, 7)
            else:
                actions = self.vla_model.predict_action(
                    **inputs, unnorm_key="bridge_orig", do_sample=(not argmax)
                ).reshape(batch_size, repeat * multiplier, 7)
    ...
    def update(self, batch: Batch, timer: Timer, pmap_axis: str = None):
        if timer is not None:
    ...
    def create(
        ...
        **kwargs,
        ...
        assert not use_quantization, "Apparently this hurts performance, be careful"
        ...
        device_id = "cuda:0"
        torch.cuda.empty_cache()
        vla_model = vla_model.to(device_id)
        ...
        if use_lora:
            ...
            vla_model = get_peft_model(vla_model, lora_config)
