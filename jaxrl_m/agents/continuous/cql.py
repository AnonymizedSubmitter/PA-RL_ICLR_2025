...
class ContinuousCQLAgent(SACAgent):
    ...
    def forward_policy_and_sample(
        ...
        **kwargs,
        ...
        action_dist = self.forward_policy(obs, rng, grad_params=grad_params, **kwargs)
        if repeat:
    ...
    def _get_cql_q_diff(
        ...
        cql_current_actions, cql_current_log_pis = self.forward_policy_and_sample(
            ...
            dataset_actions=(
                batch["actions"]
                if self.config.get("use_dataset_actions_for_cql_regularization")
                else None
            ),
        )
        chex.assert_shape(
        ...
        if not self.config.get("use_dataset_actions_for_cql_regularization"):
